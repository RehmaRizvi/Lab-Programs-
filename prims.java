...
public class prims
{
    ...
    {   
        ...
        {    
             ...
             for(j=1;j<=n;j++)
              if(this.cost[i][j]<min)
                if(this.isVisited[i]!=0)
             	...
             	{
  					  System.out.println("Edge Number "+num_edges+"-->from Vertex "+a+" to Vertex "+b+"  Min_cost-->"+min+" \n");
            ...
            System.out.println("\nTotal Minimum Cost="+minimum_cost);           
    ...
    public static void main(String args[])
    {
        ...
        nodes = in.nextInt();
        prims p = new prims();
        System.out.println("Enter the Cost/Weights: \n");
        for(i=1;i<=nodes;i++)
          for(j=1;j<=nodes;j++)
          {
            p.cost[i][j]=in.nextInt();
            if(p.cost[i][j]==0)
              p.cost[i][j]=999; //make the cost 999 so it does not visit the same node again
          }
